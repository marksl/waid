...
(function() {
    ...
    n(y.prototype, {
        getSVG: function(a) {
            ...
            if (!h.createElementNS) h.createElementNS = function(a, b) { return h.createElement(b); };
            a = r("div", null, {
                position: "absolute",
                top: "-9999em",
            ...
            d = d.replace(/zIndex="[^"]+"/g, "").replace(/isShadow="[^"]+"/g, "").replace(/symbolName="[^"]+"/g, "").replace(/jQuery[0-9]+="[^"]+"/g, "").replace(/isTracker="[^"]+"/g, "").replace(/url\([^#]+#/g, "url(#").replace(/<svg /, '<svg xmlns:xlink="http://www.w3.org/1999/xlink" ').replace(/ href=/g, " xlink:href=").replace(/\n/, " ").replace(/<\/svg>.*?$/, "</svg>").replace(/&nbsp;/g, " ").replace(/&shy;/g, "­").replace(/<IMG /g,
                "<image ").replace(/height=([^" ]+)/g, 'height="$1"').replace(/width=([^" ]+)/g, 'width="$1"').replace(/hc-svg-href="([^"]+)">/g, 'xlink:href="$1"/>').replace(/id=([^" >]+)/g, 'id="$1"').replace(/class=([^" ]+)/g, 'class="$1"').replace(/ transform /g, " ").replace(/:(path|rect)/g, "$1").replace(/style="([^"]+)"/g, function(a) { return a.toLowerCase(); });
        ...
        exportChart: function(a,
            ...
            k(["filename", "type", "width", "svg"], function(b) { r("input", { type: "hidden", name: b, value: { filename: a.filename || "chart", type: a.type, width: a.width, svg: d }[b] }, null, c); });
            c.submit();
            u(c);
        },
        print: function() {
            ...
            if (!a.isPrinting)
                a.isPrinting = !0, k(f, function(a,
                    ...
                    d.appendChild(b);
                    k(f, function(a, b) { if (a.nodeType === 1) a.style.display = c[b]; });
                    a.isPrinting = !1;
                }, 1E3);
        },
        contextMenu: function(a, b, c, d, e, f) {
            ...
            if (!j)
                i[q] = j = r("div", { className: "highcharts-" + a }, { position: "absolute", zIndex: 1E3, padding: l + "px" }, i.container), m = r("div", null, n({
                    MozBoxShadow: "3px 3px 10px #888",
                    WebkitBoxShadow: "3px 3px 10px #888",
                    boxShadow: "3px 3px 10px #888"
                }, g.menuStyle), j), w = function() { t(j, { display: "none" }); }, z(j, "mouseleave", w), k(b, function(a) {
                    if (a) {
                        var b = r("div", { onmouseover: function() { t(this, g.menuItemHoverStyle); }, onmouseout: function() { t(this, h); }, innerHTML: a.text || i.options.lang[a.textKey] }, n({ cursor: "pointer" }, h), m);
    ...
    g.Renderer.prototype.symbols.exportIcon = function(a, b, c, d) { return x(["M", a, b + c, "L", a + c, b + d, a + c, b + d * 0.8, a, b + d * 0.8, "Z", "M", a + c * 0.5, b + d * 0.8, "L", a + c * 0.8, b + d * 0.4, a + c * 0.4, b + d * 0.4, a + c * 0.4, b, a + c * 0.6, b, a + c * 0.6, b + d * 0.4, a + c * 0.2, b + d * 0.4, "Z"]); };
